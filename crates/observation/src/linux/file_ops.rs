...
/// Watches a directory tree for file create/modify/delete operations via inotify
/// (through the `notify` crate), reporting path + operation type only — per
/// `docs/design/05-privacy-model.md` §1's Level 2 file-operation-metadata signal,
/// never file *content*. There is no code path in this source that reads a
/// watched file's bytes at all.
pub struct FileOperationSource {
    // Held only to keep the underlying inotify watch alive for this source's
...
impl FileOperationSource {
    pub fn watch(path: &Path) -> Result<Self, PollError> {
        ...
        let mut watcher = notify::recommended_watcher(move |res| {
            // The receiver may already be gone if the source was dropped; a send
            // failure here is not this closure's problem to handle.
            let _ = tx.send(res);
        })
    ...
    fn poll(&mut self) -> Result<Vec<CapturedSignal>, PollError> {
        ...
        while let Ok(result) = self.receiver.try_recv() {
            let event = match result {
                Ok(event) => event,
                // A watch-backend error (e.g. a watched path was removed out from
                // under us) is not fatal to the source as a whole — skip it and
            ...
            let operation = match event.kind {
                ...
                // Access events (a file being merely read/opened) and anything
                // else `notify` reports are not "operations" in the sense this
                // signal type means to capture — deliberately filtered, not
                // forgotten.
                _ => continue,
            };
...
mod tests {
    ...
    // inotify delivery is asynchronous relative to the syscall that triggers it;
    // these tests give it a short, generous window to arrive rather than assuming
    // it's instantaneous. This is a real filesystem + real inotify watch, not a
    // mock — these exercise the actual kernel notification path.
    ...
    fn never_reads_file_contents_into_a_signal() {
        ...
        // `operation` — there is no field it could put file bytes into even if a
        // future change tried to.
        let dir = tempfile::tempdir().unwrap();
        let mut source = FileOperationSource::watch(dir.path()).unwrap();
